...
def run_backtest(history, max_matches=80, min_history=80):
    """Safe rolling backtest kept in app.py so deployment does not depend on a model.py backtest import."""
    ...
    for idx in test.index:
        ...
        actual = "Home Win" if hg > ag else "Draw" if hg == ag else "Away Win"
        records.append({
            "pred": pred["pick"], "actual": actual,
...
with st.spinner("Running historical model test..."):
    bt = run_backtest(completed_all, max_matches=80, min_history=80)
if bt.get("tested", 0):
